...
Ask Genie something like *"which region has the lowest sell-through on Bottoms this period?"* and the answer should point at South — traceable end to end from `inventory_metrics` back through the gold tables to the generator's seeded assumptions.
...
- **Bundle-owned SQL warehouse, not a `lookup:`.** The bundle creates its own small serverless warehouse rather than looking one up by name, so there's zero dependency on any pre-existing object in whoever's workspace deploys this.
...
- **Genie space built in two passes.** `serialized_space`'s JSON shape isn't documented anywhere the CLI or bundle schema exposes — even `databricks genie create-space --help` points at "call Get Genie Space on an existing space to see the shape." So the space has to be hand-configured once against real deployed tables/metric views, then snapshotted with `databricks bundle generate genie-space` and committed — see the README's "Building the Genie space" section.
