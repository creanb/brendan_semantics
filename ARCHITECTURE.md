...
- **First-ever `bundle deploy` on a workspace needs a `deploy → run → deploy` sequence, not `deploy → run`.** Found via an actual destroy-and-redeploy test (`bundle destroy` + `DROP CATALOG ... CASCADE`, then rebuilding from nothing), not from reasoning about the config in the abstract. `resources.genie_spaces.retail_semantics` is declarative and validates that every trusted-asset table/view it references already exists at `bundle deploy` time — but those tables don't exist until `bundle run semantic_layer` has executed at least once. Every deploy before this test had stale gold tables left over from earlier runs, which silently masked the ordering problem. Terraform-style partial deploys are what makes the fix cheap: the pipeline and jobs succeed in the first `deploy` regardless, `run` builds what the Genie space needs, and the second `deploy` only has to retry the one resource that failed — no different in kind from the catalog/pipeline ordering issue above, just one dependency layer deeper, and only relevant the very first time a given workspace deploys this bundle.
